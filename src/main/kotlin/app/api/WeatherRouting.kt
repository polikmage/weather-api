    ...
    routing {
        ...
        get("/weather/summary") {
            ...
            val temperature: Int = call.request.queryParameters["temperature"]?.toIntOrNull()
                ?: return@get call.respond(HttpStatusCode.BadRequest, "Missing or invalid 'temperature' parameter")
            val locations = call.request.queryParameters["locations"]?.split(",")?.mapNotNull { it.toIntOrNull() }
                ?: return@get call.respond(HttpStatusCode.BadRequest, "Missing or invalid 'locations' parameter")

            val weatherUnitType = WeatherUnitType.fromString(unit)
            val owmUnit = weatherUnitType.owmValue

            val response = weatherService.getSummary(owmUnit, temperature, locations)
        ...
        get("/weather/locations/{locationId}") {
            ...
            val location = LocationDictionary.getLocation(locationId) ?: return@get call.respond(
                ...
                "Coordinates for location: $locationId not found"
            )

            val response = weatherService.getLocationTemperatures(location, WeatherUnitType.CELSIUS)
            call.respond(response)
        }
    }
}
